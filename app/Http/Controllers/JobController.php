...
class JobController extends Controller
{
    ...
    public function vnjobs(Request $request, $job_type = '')
    {
        ...
        switch ($job_type) {
            ...
            case 'urgent':
                $query->where(function ($q) {
                    $q->where('job_packages.slug', "like", "%tin-gap-vip%")
                        ->orWhere('job_packages.slug', "like", "%tin-gap%");
                })
                    ->where(function ($q) {
                        // Điều kiện cho việc làm trong nước
                        $q->whereNotNull('job_province.province_id')
                            ->where('jobs.country', '=', 'VN'); // Thay 'VN' bằng mã quốc gia phù hợp nếu cần
                    })
