        ...
        <div class="row g-3">
            <div class="col-auto flex-grow-1">
                <div class="position-relative">
                    <input class="form-control" name="name" type="text" placeholder="{{ __('search') }}"
                        value="{{ request()->name }}">
                </div>
            </div>
            <div class="col-auto">
            ...
            <div class="col-auto">
                <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-light px-4"><i
                            class="bi bi-box-arrow-right me-2"></i>{{ __('search') }}</button>
                </div>
            </div>
        </div>
    </form>
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a class="dropdown-item"
                                                            href="{{ route($route_prefix . 'edit', ['type' => request()->type, 'admintaxonomy' => $item->id]) }}">
                                                            {{ __('edit') }}
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <form
                                                            action="{{ route($route_prefix . 'destroy', ['type' => request()->type, 'admintaxonomy' => $item->id]) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button
