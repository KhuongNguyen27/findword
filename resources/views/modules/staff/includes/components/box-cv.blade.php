<div class="box-cv">
    ...
    <div class="box-bg">
        ...
        <div class="box-info">
            <h4 class="title-cv"><a href="{{ route('staff.cv.show', $item->id) }}"
                    target="_blank">{{ $item->cv_file ?? 'Chưa có tiêu đề' }} </a> <a
                    href="{{ route('staff.cv.edit', $item->id) }}" class="edit">
            ...
            <ul class="action">
                ...
                <li><a href="http://www.facebook.com/sharer/sharer.php?u={{ route('staff.cv.show', $item->id) }}"
                        target="_blank" class="btn btn-sm bold"><i class="fa fa-solid fa-share"></i> Chia sẻ</a></li>
                <!-- <li>
                    <a class="dropdown-item btn btn-sm bold btn-download-cv"
